    ...
    /***/ (function (module, exports, __webpack_require__) {
        ...
        !function (global, factory) {
            ...
            function transitionIn(el, show, forceSkip = !1) {
                ...
                const attrs = getXAttrs(el, "transition"), showAttr = getXAttrs(el, "show")[0];
                if (showAttr && showAttr.modifiers.includes("transition")) {
                    ...
                    const enter = (directives.find(i => "enter" === i.value) || {expression: ""}).expression.split(" ").filter(i => "" !== i),
                        enterStart = (directives.find(i => "enter-start" === i.value) || {expression: ""}).expression.split(" ").filter(i => "" !== i),
                        enterEnd = (directives.find(i => "enter-end" === i.value) || {expression: ""}).expression.split(" ").filter(i => "" !== i);
            ...
            function transitionOut(el, hide, forceSkip = !1) {
                ...
                if (showAttr && showAttr.modifiers.includes("transition")) {
                    ...
                    const leave = (directives.find(i => "leave" === i.value) || {expression: ""}).expression.split(" ").filter(i => "" !== i),
                        leaveStart = (directives.find(i => "leave-start" === i.value) || {expression: ""}).expression.split(" ").filter(i => "" !== i),
                        leaveEnd = (directives.find(i => "leave-end" === i.value) || {expression: ""}).expression.split(" ").filter(i => "" !== i);
            ...
            function transitionHelper(el, modifiers, hook1, hook2, styleValues) {
                const opacityCache = el.style.opacity, transformCache = el.style.transform,
                    ...
                    transitionScale = noModifiers || modifiers.includes("scale"), stages = {
                        start() {
            ...
            function transitionClasses(el, classesDuring, classesStart, classesEnd, hook1, hook2) {
                const originalClasses = el.__x_original_classes || [], stages = {
                    start() {
            ...
            function transition(el, stages) {
                stages.start(), stages.during(), requestAnimationFrame(() => {
                    let duration = 1e3 * Number(getComputedStyle(el).transitionDuration.replace(/,.*/, "").replace("s", ""));
            ...
            function handleForDirective(component, el, expression, initialUpdate) {
                ...
                var items = component.evaluateReturnExpression(el, bunch), previousEl = el;
                items.forEach((i, index, group) => {
                    ...
                    if (currentEl && void 0 !== currentEl.__x_for_key) {
                        ...
                        delete currentEl.__x_for_key, currentEl.__x_for_alias = single, currentEl.__x_for_value = i, component.updateElements(currentEl, () => ({[currentEl.__x_for_alias]: currentEl.__x_for_value}))
                    } else {
                        const clone = document.importNode(el.content, !0);
                        el.parentElement.insertBefore(clone, currentEl), currentEl = previousEl.nextElementSibling, transitionIn(currentEl, () => {
                        }, initialUpdate), currentEl.__x_for_alias = single, currentEl.__x_for_value = i, component.initializeElements(currentEl, () => ({[currentEl.__x_for_alias]: currentEl.__x_for_value}))
                    }
            ...
            function handleAttributeBindingDirective(component, el, attrName, expression, extraVars) {
                ...
                if ("value" === attrName) if (void 0 === value && expression.match(/\./).length && (value = ""), "radio" === el.type) el.checked = el.value == value; else if ("checkbox" === el.type) if (Array.isArray(value)) {
                    let valueFound = !1;
                    value.forEach(val => {
                        val == el.value && (valueFound = !0)
                    }), el.checked = valueFound
                } else el.checked = !!value; else "SELECT" === el.tagName ? function (el, value) {
                    const arrayWrappedValue = [].concat(value).map(value => value + "");
                    Array.from(el.options).forEach(option => {
                        option.selected = arrayWrappedValue.includes(option.value || option.text)
                    })
                }(el, value) : el.value = value; else if ("class" === attrName) if (Array.isArray(value)) {
                    ...
                    const originalClasses = el.__x_original_classes || [], newClasses = value.split(" ");
            ...
            function runListenerHandler(component, expression, e, extraVars) {
                return component.evaluateCommandExpression(e.target, expression, () => _objectSpread2({}, extraVars(), {$event: e}))
            }
            ...
            class Component {
                constructor(el, seedDataForCloning = null) {
                    ...
                    this.unobservedData = seedDataForCloning || saferEval(dataExpression, {}), this.$data = this.wrapDataInObservable(this.unobservedData), this.unobservedData.$el = this.$el, this.unobservedData.$refs = this.getRefsProxy(), this.nextTickStack = [], this.unobservedData.$nextTick = callback => {
                        this.nextTickStack.push(callback)
                    }, this.showDirectiveStack = [], this.showDirectiveLastElement, initExpression && !seedDataForCloning && (this.pauseReactivity = !0, initReturnedCallback = this.evaluateReturnExpression(this.$el, initExpression), this.pauseReactivity = !1), this.initializeElements(this.$el), this.listenForNewElementsToInitialize(), "function" == typeof initReturnedCallback && initReturnedCallback.call(this.$data)
                }

                getUnobservedData() {
                    let rawData = {};
                    return Object.keys(this.unobservedData).forEach(key => {
                ...
                executeAndClearRemainingShowDirectiveStack() {
                    ...
                    })).reduce((nestedPromise, promise) => nestedPromise.then(() => promise.then(finish => finish())), Promise.resolve(() => {
                        ...
                        switch (type) {
                            ...
                            case"model":
                                !function (component, el, modifiers, expression, extraVars) {
                                    ...
                                    registerListener(component, el, event, modifiers, `${expression} = rightSideOfExpression($event, ${expression})`, () => _objectSpread2({}, extraVars(), {rightSideOfExpression: generateModelAssignmentFunction(el, modifiers, expression)}))
                            ...
                            case"show":
                                ...
                                !function (component, el, value, modifiers, initialUpdate = !1) {
                                    ...
                                    const handle = resolve => {
                                        ...
                                        })) : "none" !== el.style.display ? transitionOut(el, () => {
                                            resolve(() => {
                                                hide()
                                            })
                                        }) : resolve(() => {
                            ...
                            case"if":
                                ...
                                !function (el, expressionResult, initialUpdate) {
                                    ...
                                    if (expressionResult && !elementHasAlreadyBeenAdded) {
                                        const clone = document.importNode(el.content, !0);
                                        el.parentElement.insertBefore(clone, el.nextElementSibling), el.nextElementSibling.__x_inserted_me = !0, transitionIn(el.nextElementSibling, () => {
                                        }, initialUpdate)
                ...
                evaluateReturnExpression(el, expression, extraVars = (() => {
                    ...
                    return saferEval(expression, this.$data, _objectSpread2({}, extraVars(), {$dispatch: this.getDispatchFunction(el)}))
                }
...
// });
  ...
  \***********************************************************/
    ...
    /***/ (function (module, exports, __webpack_require__) {
        ...
        __webpack_require__(/*! /Users/waveriding/Sites/cit/resources/js/app.js */"./resources/js/app.js");
        module.exports = __webpack_require__(/*! /Users/waveriding/Sites/cit/resources/css/app.css */"./resources/css/app.css");
