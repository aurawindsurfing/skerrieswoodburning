...
<body>
    ...
    @foreach ($bookings as $booking)
        ...
        <div class="container ml-6">
            ...
            <b>{{ isset($booking->upcoming_course_dates()->first()->course->venue->google_maps) ? 'Directions: ' : ''}}</b><a href="{{ isset($booking->upcoming_course_dates()->first()->course->venue->google_maps) ? $booking->upcoming_course_dates()->first()->course->venue->google_maps : '' }}">{{ isset($booking->upcoming_course_dates()->first()->course->venue->google_maps) ? $booking->upcoming_course_dates()->first()->course->venue->google_maps : '' }}</a><br><br><br>
        </div>
    ...
    @foreach ($booking->upcoming_course_dates() as $additional_date)
        ...
        <div class="container ml-6">
            ...
            <b>{{ isset($additional_date->venue->google_maps) ? 'Directions: ' : ''}}</b><a href="{{ isset($additional_date->venue->google_maps) ? $additional_date->venue->google_maps : '' }}">{{ isset($additional_date->venue->google_maps) ? $additional_date->venue->google_maps : '' }}</a><br><br><br>
        </div>
